...
describe("account cli", () => {
  ...
  it("add command ignores unreadable integration state when explicit router-state and auth-store are supplied", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "account-cli-explicit-paths-"));
    cleanupPaths.push(dir);
  ...
  it("list command does not resolve auth-store when router-state is explicit", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "account-cli-router-only-"));
    cleanupPaths.push(dir);
